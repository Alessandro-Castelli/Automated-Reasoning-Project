...
input_dir = os.path.join(current_dir, "instancesMinizinc", "medium")
output_dir = os.path.join(current_dir, "output Medium Minizinc")    

# Crea la cartella di output se non esiste
os.makedirs(output_dir, exist_ok=True)

# Funzione per eseguire il modello MiniZinc e misurare il tempo di esecuzione
def run_minizinc(model, data_file, solver, time_limit):
    start_time = time.time()
    result = subprocess.run(
        ["minizinc", "--solver", solver, "--time-limit", str(time_limit), model, data_file],
        capture_output=True,
        text=True
    )
    end_time = time.time()
...
for filename in os.listdir(input_dir):
    if filename.endswith(".dzn"):
       ...
       # Esegui per Gecode
        output_gecode, error_gecode, return_code_gecode, exec_time_gecode = run_minizinc(model_path, input_path, "Gecode", 300000)  # Esempio: limite di tempo di 300 secondi (5 minuti)
        
        # Esegui per Chuffed
        output_chuffed, error_chuffed, return_code_chuffed, exec_time_chuffed = run_minizinc(model_path, input_path, "Chuffed", 300000)
